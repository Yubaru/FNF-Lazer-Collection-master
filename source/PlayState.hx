...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'city': //Week 1
				var citybg:BGSprite = new BGSprite('cityscape', -600, -200, 0.9, 0.9);
				add(citybg);

				case 'genieroom': //Week 1
				var geniebg:BGSprite = new BGSprite('genieroom', 100, -100, 0.9, 0.9);
				add(geniebg);

				case 'nyu': //Week 1
				var nyubg:BGSprite = new BGSprite('nyu', -600, -200, 0.9, 0.9);
				add(nyubg);
				...
				if (SONG.song.toLowerCase() == 'Dirty Mind')
					gf.alpha = 0;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'genieIntro':
				var genieIntro:FlxSprite = new FlxSprite();

				genieIntro.loadGraphic(Paths.image('genieIntro'));

					genieIntro.antialiasing = true;

					genieIntro.active = false;

					genieIntro.scrollFactor.set();

					genieIntro.screenCenter();

					add(genieIntro);

					FlxTween.tween(genieIntro, {alpha: 0},1, {
			...
			case 'genieOutro':
				var genieOutro:FlxSprite = new FlxSprite();

				genieOutro.loadGraphic(Paths.image('genieOutro'));

					genieOutro.antialiasing = true;

					genieOutro.active = false;

					genieOutro.scrollFactor.set();

					genieOutro.screenCenter();

					add(genieOutro);

					FlxTween.tween(genieOutro, {alpha: 0},1, {
		...
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					case 'Dodge Note': //Hurt note //Just remember that sheila doesnt know how to code LOL skill issue
